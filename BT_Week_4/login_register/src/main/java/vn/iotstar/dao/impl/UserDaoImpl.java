...
//		}
	...
	@Override
	public void UpdateByUser(String user, String fullname, int roleid, String phone) {
		// TODO Auto-generated method stub
		String updateSQL = "UPDATE ltwebt5 SET fullname = ?, roleid = ?, phone = ? WHERE user = ?";
		try {
			conn = super.getDatabaseConnection();
            PreparedStatement ps = conn.prepareStatement(updateSQL);
            // Thiết lập các giá trị cần cập nhật
            ps.setString(1, fullname);  // Cập nhật tên
            ps.setInt(2, roleid);       // Cập nhật roleid
            ps.setString(3, phone);     // Cập nhật số điện thoại
            ps.setString(4, user);      // Điều kiện WHERE cho người dùng
